...
class ConfigRelayGetConfiguratorState: NSObject, ConfiguratorStateProtocol {
    ...
    func execute() {
        let message = ConfigRelayGetMessage()
        ...
        for aPayload in payloads! {
            ...
            if data.count <= target.basePeripheral().maximumWriteValueLength(for: .withoutResponse) {
                print("Sending config relay get data: \(data.hexString())")
