...
class ConfigNetworkTransmitSetConfiguratorState: NSObject, ConfiguratorStateProtocol {
    
    // MARK: - Properties
    private var proxyService            : CBService!
    private var networkTransmitCount    : Int!
    private var networkTransmitIntervalSteps : Int!
    ...
    public func setNetworkTransmit(withNetworkTransmitCount networkTransmitCount: Int,
                                   withNetworkTransmitIntervalSteps networkTransmitIntervalSteps: Int) {
        
        self.networkTransmitCount = networkTransmitCount;
        self.networkTransmitIntervalSteps = networkTransmitIntervalSteps;
    }
    ...
    func execute() {
        let message = ConfigNetworkTransmitSetMessage(withNetworkTransmitCount: networkTransmitCount, withNetworkTransmitIntervalSteps: networkTransmitIntervalSteps)
        ...
        for aPayload in payloads! {
            ...
            if data.count <= target.basePeripheral().maximumWriteValueLength(for: .withoutResponse) {
                print("Sending config network transmit get data: \(data.hexString())")
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is ConfigNetworkTransmitStatusMessage {
                    let configRelayStatus = result as! ConfigNetworkTransmitStatusMessage
                    print("configNetworkTransmitStatus: \(configRelayStatus.debugDescription)")
                    target.delegate?.receivedNetworkTransmitStatus(configRelayStatus)
                } else {
