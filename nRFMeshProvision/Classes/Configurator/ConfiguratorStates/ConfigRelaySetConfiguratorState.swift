...
class ConfigRelaySetConfiguratorState: NSObject, ConfiguratorStateProtocol {
    ...
    private var relayRetransmitCount: Int!
    private var relayRetransmitIntervalSteps: Int!
    ...
    public func setRelay(withRelay relay: Int,
                         withRelayRetransmitCount relayRetransmitCount: Int,
                         withRelayRetransmitIntervalSteps relayRetransmitIntervalSteps: Int) {
        self.relay = relay;
        self.relayRetransmitCount = relayRetransmitCount;
        self.relayRetransmitIntervalSteps = relayRetransmitIntervalSteps;
    }
    ...
    func execute() {
        let message = ConfigRelaySetMessage(withRelay: relay, withRelayRetransmitCount: relayRetransmitCount, withRelayRetransmitIntervalSteps: relayRetransmitIntervalSteps);
        ...
        for aPayload in payloads! {
            ...
            if data.count <= target.basePeripheral().maximumWriteValueLength(for: .withoutResponse) {
                print("Sending config relay set data: \(data.hexString())")
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is ConfigRelayStatusMessage {
                    let configRelayStatus = result as! ConfigRelayStatusMessage
                    print("configRelayStatus: \(configRelayStatus.debugDescription)")
                    ...
                    */
                    target.delegate?.receivedRelayConfig(configRelayStatus)
                } else {
