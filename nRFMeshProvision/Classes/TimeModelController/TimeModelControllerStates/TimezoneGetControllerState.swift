...
class TimezoneGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Time Zone Get"
    }

    func execute() {
        let message = TimezoneGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non TimezoneStatus message")
            }
        }
    }
