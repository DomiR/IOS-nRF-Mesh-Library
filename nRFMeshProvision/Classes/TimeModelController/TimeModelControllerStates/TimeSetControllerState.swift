...
class TimeSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: TimeSetMessage
        if let targetState = payload {
            message = TimeSetMessage(withPayload: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is TimeStatusMessage {
                    let timeStatus = result as! TimeStatusMessage
                    target.delegate?.receivedTimeStatusMessage(timeStatus)
                ...
                print("ignoring non TimeStatusMessage")
            }
        }
    }
