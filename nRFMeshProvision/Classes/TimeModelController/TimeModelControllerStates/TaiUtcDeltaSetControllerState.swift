//
//  TaiUtcDeltaSetControllerState.swift
//  nRFMeshProvision
//
...
class TaiUtcDeltaSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: TaiUtcDeltaSetMessage
        if let targetState = payload {
            message = TaiUtcDeltaSetMessage(withPayload: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non TaiUtcDeltaStatusMessage")
            }
        }
    }
