//
//  TimezoneSetControllerState.swift
//  nRFMeshProvision
//
...
class TimezoneSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: TimezoneSetMessage
        if let targetState = payload {
            message = TimezoneSetMessage(withPayload: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is TimezoneStatusMessage {
                    let timeZoneStatus = result as! TimezoneStatusMessage
                    target.delegate?.receivedTimezoneStatusMessage(timeZoneStatus)
                ...
                print("ignoring non TimeMessage")
            }
        }
    }
