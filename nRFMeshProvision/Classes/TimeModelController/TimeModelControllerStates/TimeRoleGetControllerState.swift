...
class TimeRoleGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Time Role Get"
    }

    func execute() {
        let message = TimeRoleGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non TimeRoleStatus message")
            }
        }
    }
