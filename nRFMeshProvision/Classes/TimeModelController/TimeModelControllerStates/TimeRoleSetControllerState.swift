//
//  TimeRoleSetControllerState.swift
//  nRFMeshProvision
//
...
class TimeRoleSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: TimeRoleSetMessage
        if let targetState = payload {
            message = TimeRoleSetMessage(withPayload: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is TimeRoleStatusMessage {
                    let timeRoleStatus = result as! TimeRoleStatusMessage
                    target.delegate?.receivedTimeRoleStatusMessage(timeRoleStatus)
                ...
                print("ignoring non TimeRoleMessage")
            }
        }
    }
