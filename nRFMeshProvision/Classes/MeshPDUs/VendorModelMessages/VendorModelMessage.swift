...
public struct VendorModelMessage {
    var opcode  : Data
    var payload : Data
    var accessMsg: AccessMessagePDU?
    ...
    public mutating func assemblePayload(withMeshState aState: MeshState, toAddress aDestinationAddress: Data) -> [Data]? {
        ...
        accessMsg = AccessMessagePDU(withPayload: payload, opcode: opcode, appKey: appKey, netKey: aState.netKeys[0].key, seq: SequenceNumber(), ivIndex: aState.netKeys[0].phase, source: aState.unicastAddress, andDst: aDestinationAddress)
        let networkPDU = accessMsg!.assembleNetworkPDU()
        return networkPDU
    }
}
