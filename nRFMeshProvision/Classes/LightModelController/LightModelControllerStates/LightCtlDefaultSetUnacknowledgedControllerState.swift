...
class LightCtlDefaultSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Ctl Default Unacknowled Set"
    }
    ...
    func execute() {
        var message: LightCtlDefaultSetUnacknowledgedMessage
        if let targetState = targetState {
            message = LightCtlDefaultSetUnacknowledgedMessage(withTargetState: targetState)
        } else {
        ...
        target.delegate?.sentLightCtlDefaultSetUnacknowledged(destinationAddress);
