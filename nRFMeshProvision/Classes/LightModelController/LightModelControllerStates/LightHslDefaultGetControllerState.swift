...
class LightHslDefaultGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Hsl Default Get"
    }

    func execute() {
        let message = LightHslDefaultGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non LightHslDefaultStatus message")
            }
        }
    }
