...
class LightCtlDefaultSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: LightCtlDefaultSetMessage
        if let targetState = targetState {
            message = LightCtlDefaultSetMessage(withTargetState: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightCtlDefaultStatusMessage {
                    let lightCtlDefaultStatus = result as! LightCtlDefaultStatusMessage
                    target.delegate?.receivedLightCtlDefaultStatusMessage(lightCtlDefaultStatus)
                ...
                print("ignoring non LightCtlDefaultMessage")
            }
        }
    }
