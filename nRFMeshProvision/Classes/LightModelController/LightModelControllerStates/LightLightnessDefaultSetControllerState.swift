...
class LightLightnessDefaultSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Lightness Default Set"
    }
    ...
    func execute() {
        var message: LightLightnessDefaultSetMessage
        if let targetState = targetState {
            message = LightLightnessDefaultSetMessage(withTargetState: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightLightnessDefaultStatusMessage {
                    let lightLightnessDefaultStatus = result as! LightLightnessDefaultStatusMessage
                    target.delegate?.receivedLightLightnessDefaultStatusMessage(lightLightnessDefaultStatus)
