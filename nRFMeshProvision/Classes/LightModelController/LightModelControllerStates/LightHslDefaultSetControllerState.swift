//
//  LightHslDefaultSetControllerState.swift
//  nRFMeshProvision
//
...
class LightHslDefaultSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: LightHslDefaultSetMessage
        if let targetState = targetState {
            message = LightHslDefaultSetMessage(withTargetState: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is LightHslDefaultStatusMessage {
                    let lightHslDefaultStatus = result as! LightHslDefaultStatusMessage
                    target.delegate?.receivedLightHslDefaultStatusMessage(lightHslDefaultStatus)
                ...
                print("ignoring non LightHslDefaultMessage")
            }
        }
    }
