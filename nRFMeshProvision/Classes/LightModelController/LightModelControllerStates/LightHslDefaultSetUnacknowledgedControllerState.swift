...
class LightHslDefaultSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Hsl Default Unacknowled Set"
    }
    ...
    func execute() {
        var message: LightHslDefaultSetUnacknowledgedMessage
        if let targetState = targetState {
            message = LightHslDefaultSetUnacknowledgedMessage(withTargetState: targetState)
        } else {
        ...
        target.delegate?.sentLightHslDefaultSetUnacknowledged(destinationAddress);
