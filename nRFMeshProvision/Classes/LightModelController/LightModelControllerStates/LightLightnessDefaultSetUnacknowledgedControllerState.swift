...
class LightLightnessDefaultSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Lightness Default Unacknowledged Set"
    }
    ...
    func execute() {
        var message: LightLightnessDefaultSetUnacknowledgedMessage
        if let targetState = targetState {
            message = LightLightnessDefaultSetUnacknowledgedMessage(withTargetState: targetState)
        } else {
        ...
        target.delegate?.sentLightLightnessDefaultSetUnacknowledged(destinationAddress);
