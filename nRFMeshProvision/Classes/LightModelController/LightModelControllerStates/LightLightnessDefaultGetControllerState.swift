...
class LightLightnessDefaultGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Light Lightness Default Get"
    }

    func execute() {
        let message = LightLightnessDefaultGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non LightLightnessDefaultStatus message")
            }
        }
    }
