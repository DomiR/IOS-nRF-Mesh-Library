...
class HealthFaultGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message: HealthFaultGetMessage
        if let targetState = targetState {
            message = HealthFaultGetMessage(withTargetState: targetState)
        } else {
