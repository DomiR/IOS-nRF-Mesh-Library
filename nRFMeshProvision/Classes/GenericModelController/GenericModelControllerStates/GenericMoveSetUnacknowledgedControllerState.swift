//
//  GenericMoveSetControllerState.swift
//  nRFMeshProvision
//
...
class GenericMoveSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: GenericMoveSetUnacknowledgedMessage
        if let targetState = targetState {
            if let targetTransitionParams = targetStateTransitionParameters {
                message = GenericMoveSetUnacknowledgedMessage(withTargetState: targetState,
        ...
        target.delegate?.sentGenericMoveSetUnacknowledged(destinationAddress);
