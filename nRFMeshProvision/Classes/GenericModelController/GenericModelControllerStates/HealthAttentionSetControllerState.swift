//
//  HealthAttentionSetControllerState.swift
//  nRFMeshProvision
//
...
class HealthAttentionSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        var message: HealthAttentionSetMessage
        if let targetState = targetState {
            message = HealthAttentionSetMessage(withTargetState: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is HealthAttentionStatusMessage {
                    let HealthAttentionStatus = result as! HealthAttentionStatusMessage
                    target.delegate?.receivedHealthAttentionStatusMessage(HealthAttentionStatus)
                ...
                print("ignoring non HealthAttentionStatus message")
            }
        }
    }
