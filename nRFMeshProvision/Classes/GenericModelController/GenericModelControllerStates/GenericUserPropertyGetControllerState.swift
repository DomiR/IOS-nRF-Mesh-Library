//
//  GenericUserPropertyGetControllerState.swift
//  nRFMeshProvision
//
...
class GenericUserPropertyGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    public func setUserPropertyKey(aTargetKey: Data) {
        targetState = aTargetKey
    }

    func execute() {
        var message: GenericUserPropertyGetMessage
        if let targetState = targetState {
            message = GenericUserPropertyGetMessage(withTargetState: targetState)
        } else {
