//
//  HealthFaultGetControllerState.swift
//  nRFMeshProvision
//
...
class HealthFaultTestControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    private var testId                  : Data?
    private var companyId               : Data?
    ...
    public func setTestId(aTestId: Data) {
        testId = aTestId
    }
    
    public func setCompanyId(aCompanyId: Data) {
    ...
    func execute() {
        let message: HealthFaultTestMessage
        if let testId = testId, let companyId = companyId {
            message = HealthFaultTestMessage(withTestId: testId, withCompanyId: companyId)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is HealthFaultStatusMessage {
                    let healthFaultStatus = result as! HealthFaultStatusMessage
                    target.delegate?.receivedHealthFaultStatusMessage(healthFaultStatus)
                ...
                print("ignoring non HealthAttentioinStatus message")
            }
        }
    }
