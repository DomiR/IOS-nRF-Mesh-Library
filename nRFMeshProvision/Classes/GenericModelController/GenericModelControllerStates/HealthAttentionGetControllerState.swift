//
//  HealthAttentionGetControllerState.swift
//  nRFMeshProvision
//
...
class HealthAttentionGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message = HealthAttentionGetMessage()
        //Send to destination
        ...
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is HealthAttentionStatusMessage {
                    let healthAttentioinStatus = result as! HealthAttentionStatusMessage
                    target.delegate?.receivedHealthAttentionStatusMessage(healthAttentioinStatus)
