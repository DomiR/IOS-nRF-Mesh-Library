//
//  SchedulerActionGetControllerState
//  nRFMeshProvision
//
...
class SchedulerActionGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Scheduler Action Get"
    }
  
    public func setIndex(aIndex: Data) {
      index = aIndex
    }


    func execute() {
        let message: SchedulerActionGetMessage
        if let index = index {
          message = SchedulerActionGetMessage(withIndex: index)
        } else {
          print("No index set, nothing to execute")
          return
        }
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SchedulerActionStatusMessage {
                    let schedulerActionStatus = result as! SchedulerActionStatusMessage
                    target.delegate?.receivedSchedulerActionStatusMessage(schedulerActionStatus)
                ...
                print("ignoring non SchedulerActionStatusMessage message")
            }
        }
    }
