...
class SchedulerGetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func execute() {
        let message = SchedulerGetMessage()
        //Send to destination
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                if result is SchedulerStatusMessage {
                    let schedulerStatus = result as! SchedulerStatusMessage
                    target.delegate?.receivedSchedulerStatusMessage(schedulerStatus)
                ...
                print("ignoring non SchedulerStatus message")
            }
        }
    }
