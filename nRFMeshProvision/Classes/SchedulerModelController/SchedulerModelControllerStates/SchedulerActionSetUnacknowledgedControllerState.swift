...
class SchedulerActionSetUnacknowledgedControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    func humanReadableName() -> String {
        return "Scheduler Action Unacknowled Set"
    }
    ...
    func execute() {
        var message: SchedulerActionSetUnacknowledgedMessage
        if let targetState = targetState {
            message = SchedulerActionSetUnacknowledgedMessage(withPayload: targetState)
        } else {
        ...
        target.delegate?.sentSchedulerActionSetUnacknowledged(destinationAddress);
