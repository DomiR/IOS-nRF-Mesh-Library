//
//  SchedulerActionSetControllerState.swift
//  nRFMeshProvision
//
...
class SchedulerActionSetControllerState: NSObject, GenericModelControllerStateProtocol {
    ...
    public func setTargetState(aPayload: Data) {
        payload = aPayload
    }

    func execute() {
        var message: SchedulerActionSetMessage
        if let targetState = payload {
            message = SchedulerActionSetMessage(withPayload: targetState)
        } else {
    ...
    func receivedData(incomingData : Data) {
        if incomingData[0] == 0x01 {
            ...
            if let result = networkLayer.incomingPDU(strippedOpcode) {
                ...
                print("ignoring non SchedulerActionStatusMessage")
            }
        }
    }
